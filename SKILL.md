---
name: drawlang
description: 绘语 - 中文优先的 AI 配图工具。支持 Style × Layout 二维矩阵设计，ModelScope/Gemini 多提供商，专为中文内容创作者优化。三种模式：(1) 文章配图 - 智能分析生成插图；(2) PPT/Slides - 批量信息图；(3) Cover - 封面图。支持小红书/公众号/B站等中文平台。触发词：配图、插图、绘语、drawlang、小红书、公众号、PPT、slides、封面图。
---
...
/drawlang --mode cover --platform wechat --topic "AI 绘图工具评测"
/drawlang article.md --mode cover --platform xiaohongshu --style fresh
```
...
| 平台 | 代码 | 宽高比 | 适用场景 |
|------|------|--------|---------|
| 公众号 | `wechat` | 2.35:1 | 封面图 |
| 小红书 | `xiaohongshu` | 3:4 | 竖图 |
| B站 | `youtube` | 16:9 | 视频封面 |
| 知乎 | `landscape` | 16:9 | 文章配图 |
...
| `--platform` | `youtube` | 平台尺寸（仅 cover 模式）：wechat, xiaohongshu, youtube, landscape |
