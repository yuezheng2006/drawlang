...
> **🆕 v1.5.0 — ModelScope Z-Image-Turbo 支持（2026 年 3 月）**
>
> 新增 ModelScope Z-Image-Turbo 作为第三个图像生成提供商，支持中英文双语。现在支持三个提供商：Gemini、ModelScope 和 OpenRouter。[查看详情 →](#图像生成提供商)
...
Claude Code 的智能文章配图 Skill，采用**三引擎系统**和**多提供商支持**：根据内容类型和可用性自动选择最佳渲染引擎（Gemini/Excalidraw/Mermaid）和图像生成提供商（Gemini/ModelScope/OpenRouter）。

## ✨ 本分支的特色功能

本分支在原版 smart-illustrator 基础上扩展了：

- **🇨🇳 ModelScope Z-Image-Turbo 集成**：原生中文支持，快速推理
- **🔄 多提供商灵活性**：自动检测并在 OpenRouter、ModelScope 和 Gemini 之间切换
- **💰 成本优化**：ModelScope API 提供免费额度
- **🚀 异步任务处理**：高效的轮询式图像生成
- **🔒 向后兼容**：保留所有现有 Gemini 和 OpenRouter 功能
...
为文章配图很费时间：手动设计需要数小时，图库图片缺乏定制性，通用 AI 工具不理解文章结构。Smart Illustrator 结合智能位置检测、三引擎系统（Gemini + Excalidraw + Mermaid）、多提供商支持（Gemini + ModelScope + OpenRouter）和封面学习，几分钟内生成符合上下文的插图。

**适合谁用：** Newsletter 作者、YouTube 创作者、技术博客作者、课程讲师，特别是创作中文内容或需要双语支持的用户。

**什么时候用：** 需要为文章生成高质量插图、基于最佳实践生成 YouTube 封面图、保持内容系列视觉风格一致、或需要中文支持的成本优化图像生成时。
...
- **多提供商支持**：可选择 Gemini、ModelScope Z-Image-Turbo 或 OpenRouter
...
- **双语支持**：ModelScope 原生支持中英文文本渲染
- **成本优化**：ModelScope API 提供免费额度
- **多后端支持**：Gemini API 生成创意视觉（2K 分辨率）、ModelScope 快速双语生成、Excalidraw 生成手绘图表、Mermaid CLI 生成结构化图表 — 全部默认输出 PNG

## 图像生成提供商

本工具支持三个图像生成提供商，并支持自动切换：

| 提供商 | 最适合 | 优点 | 缺点 |
|--------|--------|------|------|
| **Gemini** | 创意视觉、高质量插图 | 质量优秀、2K 分辨率、细节丰富 | 付费 API（$0.134/张） |
| **ModelScope** | 中文内容、双语项目、成本敏感 | 免费额度、快速推理、原生中文支持 | 异步处理（需轮询） |
| **OpenRouter** | 消费控制、API 聚合 | 可设置消费限额、统一接口 | 价格与 Gemini 相同 |

**自动检测优先级**：OpenRouter → ModelScope → Gemini（基于可用的 API 密钥）
